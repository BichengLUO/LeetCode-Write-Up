...
\bigvee\limits_{k\geqslant 0}{T(i-k,j-2)\vee \bm{E}(s[i-k+1,\cdots,i],p[j-1])} & p[j]=\text{`*'}
\end{cases}
\end{equation*}
$T(i,j)$ denotes whether the string $s[1,\cdots,i]$ matches with $p[1,\cdots,j]$. The function $\bm{E}(s[i,\cdots,j],c)$ indicates whether the substring $s[i,\cdots,j]$ matches the element $c$, which should be defined as:
\begin{equation*}
\bm{E}(s[i,\cdots,j],c)=\begin{cases}
\textbf{true} & i > j \text{ or } c=\text{`.'}\\
\bigvee\limits^{j}_{k=i}{s[k]=c} & \text{others}
\end{cases}
\end{equation*}
